...
Class Style_specification extends CI_Controller { 
	...
	function table_spec_detail(){
		...
		//colNames:['#','DETAIL_ID','ID','SIZE','POINT OF MEASURE','SIZE S', 'SIZE M', 'SIZE L', 'SIZE XL', 'NOTE','#'],
		$field = array();
	...
	//	$nav_button[] = array('method_id' => 1077,'title' => 'Copy Specification', 'icon' => 'fa fa-clone', 'load' => 'sample/style_specification/function_add');
   ...
   function loaddata_detail_coba() {
	...
	//	$style_spec_header_id = isset($_POST['style_spec_header_id']) ? $_POST['style_spec_header_id'] : 1;
		...
		//var_dump($loaddata['data']);
		foreach($loaddata['data'] as $key => $value){
				$this_order[$key] = 0;
				
				//$nilai=$this->mainconfig->fractionToDecimal($value[4]);
				//var_dump($nilai);
			//	var_dump($value[1]);
			//$this->mainconfig->decimalToFraction($decimal) 
   ...
   function loaddata_select() {
	 //  "1:Entity1;2:Entity2;3:Entity3;4:Entity4"
	   $data = array();
	   $data[] = "1:Entity1";
	   $data[] = "2:Entity2";
	   $data[] = "3:Entity3";
   ...
   function loaddata_history() {
		$this->authentication->plainlayout();
		//var_dump($_REQUEST);
		//$spec_header_id= isset($_REQUEST['style_spec_header_id']) ? is_numeric($_REQUEST['style_spec_header_id']) ? $_REQUEST['style_spec_header_id']  : -1 : -1;
	...
	function loaddata_detail() {
		...
		//$style_spec_detail_id = isset($_REQUEST['style_spec_detail_id']) ? is_numeric($_REQUEST['style_spec_detail_id']) ? $_REQUEST['style_spec_detail_id']  : -1 : -1;
		$style_spec_header_id = isset($_REQUEST['style_spec_header_id']) ? is_numeric($_REQUEST['style_spec_header_id']) ? $_REQUEST['style_spec_header_id']  : -1 : -1;
		$methodid = isset($_REQUEST['methodid']) ? is_numeric($_REQUEST['methodid']) ? $_REQUEST['methodid']  : -1 : -1;
		...
		//var_dump($cek);die();
		 ...
		 // if(count($_POST) > 0){
			...
			//var_dump($dt[0]['style_spec_detail_measure']);die();
			...
			foreach($isi_data as $dt_data){
				...
				//$x='3/4';
			//	$nilai= $this->mainconfig->fractionToDecimal($x);
				//$nilai= $this->mainconfig->fractionToDecimal($dt_data['nilai_spec']);
				//var_dump($nilai);die();
