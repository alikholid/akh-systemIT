...
Class Loader_pop extends CI_Controller { 
	...
	function index() {
		...
		$data_list['data_karyawan'] = array('view' => 'dbo.view_karyawan','view_detail' => 'dbo.view_karyawan_detail','view_detail_session' => 'karyawan','detail_session_key' => 'karyawan_index','detail_session_seq' => 'karyawan_seq', 'key' => 'karyawan_id');
		...
		if(in_array($param,$get)){
			if($param == 'get_item_detail'){
				...
				$data_table = $this->main->getData_pop('dbo.view_list_item_detail', null, $where, null, null, 10);
				if($data_table){
			...
			if($param == 'get_work_order_plan_process'){
				...
				$data_table = $this->main->getData_pop('dbo.view_list_work_order_plan_process', null, $where, null, $order );
			...
			if($param == 'get_contract_subcon_type'){
				...
				$data_table = $this->main->getData_pop('dbo.view_contract_subcon', null, $where, null, $order , 1);
				if($data_table){
					$return = $data_table;
				} else {
					$return = false;
			...
			if($param == 'get_good_receive_no'){
				...
				$data_table = $this->main->getData_pop('dbo.view_get_bc_no_po_no', null, $where, null, $order , 1);
				if($data_table){
					foreach($data_table as $key => $value){
						$bc_no = $value['doc_no'];
						$po_no = $value['po_no'];
			...
			if($param == 'get_bank_curr'){
				...
				$data_table = $this->main->getData_pop('dbo.view_bank', null, $where, null, $order , 1);
				if($data_table){
			...
			if($param == 'get_purchase_data'){
				...
				$data_table = $this->main->getData_pop('dbo.view_purchase_data', null, $where, null, $order , 1);
				if($data_table){
					$return = $data_table;
				...
				if($home_currency == $currencies_id){
					...
					$data_table = $this->main->getData_pop('dbo.view_currencies_rate', null, $where, null, null , 1);
					if($data_table){
						$return2 = $data_table;
			...
			if($param == 'get_sales_data'){
				...
				$data_table = $this->main->getData_pop('dbo.view_sales_data', null, $where, null, $order , 1);
				if($data_table){
					$return = $data_table;
					$return[0]['unit_price'] = $this->mainconfig->get_decimal_format2($return[0]['unit_price'],4);
					$return[0]['conversion'] = $this->mainconfig->get_decimal_format2($return[0]['conversion']);
			...
			if($param == 'get_grn_custom'){
				...
				$data_table = $this->main->getData_pop('dbo.view_list_grn_custom', null, $where, null, $order , $limit, $offset);
				if($data_table){
					foreach($data_table as $key => $value){
						$return[] = array("id"=>$value['id'],"value"=>$value['value'],"text"=>$value['text']);
					}
