...
Class Authentication  {
	function __construct(){
		...
		$role_username_config = $config['role_username_config'];
		
		
		$this->role_config = $role_config;
		$this->role_username_config = $role_username_config;
		$dir_template = "template";
		$template = $base_config['template'];
		$path_template = $base_config['template'];
		
		
		//print_r($this->CI->session->userdata('api_token'));
		...
		$this->class = $class;
		$this->method = $method;
		//var_dump( $this->CI->session->userdata());die();
		...
		$methodid = 0;
		$tokenid = 0;
	//	var_dump($method_config[$classid]);
	...
	//echo $role_id;
		...
		// print_r($tokenid);
		//print_r($role_username_config);
	//	print_r($user_id);
		//print_r($role_config[$role_id]);
		//var_dump($tokenid);
		//var_dump($role_config[$role_id]);die();
		$securityid = 0;
		if ($role_id == '-99'){
			$securityid = 1;
			...
			if(array_key_exists($role_id,$role_config)){
				...
				if(array_key_exists($tokenid, $role_username_config[$user_id])){
					$securityid = 1;
				}
				
			} 
		}
		//print_r($securityid.'-'.$tokenid);
		...
		if($module != $api_module){
			...
			if(!$maintenance_mode) {
				//var_dump($tokenid);
			//	var_dump($methodid);
				//var_dump($this->CI->session->userdata('login'));
				//print_r($methodid);
				if($methodid != 0){		
					...
					if($tokenid == -'3'){
						...
						if(!$this->CI->session->userdata('login')){
							if(isset($_SERVER['HTTP_X_REQUESTED_WITH'])){
							//	$_SERVER['HTTP_X_REQUESTED_WITH']='XMLHttpRequest2';
							   if($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest'){
								   //"XMLHttpRequest"
								  // var_dump("kesini");
									$return = array();
						...
						//var_dump(" masukx ".$securityid);die();
						//print_r(" masukx ".$securityid);
						if($securityid == '0'){
							if(!$this->CI->session->userdata('login')){
								$this->loginlayout();
							} else {
					...
					//var_dump($methodid);die();
				//	var_dump("coba ".$methodid);
					if($this->CI->session->userdata('role_id') != -99){
						//print_r($this->CI->session->userdata('role_id'));
					//print_r($_SERVER);
						if(isset($_SERVER['HTTP_X_REQUESTED_WITH'])){
							if($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest'){
								$return = array();
	...
	// var_dump($_SERVER['HTTP_X_REQUESTED_WITH']);
		if(isset($_SERVER['HTTP_X_REQUESTED_WITH'])){
			if($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest'){
				...
				//print_r($this->data['path_template'] ."/ajax");
				//print_r($this->data['path_template'] ."/ajax");
	...
	function loginlayout($array=array()){
		//var_dump($this->data['path_template']);
		$data['this'] = $this->CI;
		$data['class_uri'] = $this->generate_class_uri();
	...
	function cplayout($array=array()){
		//print_r($this->data['path_template']);
		//var_dump($this->data['path_template']
		$data['menu'] = '';
		$data['class_uri'] = $this->generate_class_uri();
	...
	function generate_menu($menu=""){
		...
		//var_dump($this->data['menu_config']);
	...
	//	var_dump($method_id);
		if($method_id != 0){
			$data_method = $this->data['data_method'];
			//var_dump($data_method[$method_id]);
			if(isset($data_method[$method_id])){
				$dt_menu = $data_method[$method_id];
				$token_id = $dt_menu['token_id'];
				//var_dump($this->role_config);
				if($token_id == -2){
					if(!$this->CI->session->userdata('login')){
						$permission = true;
					}
	...
	function permission_check_pop($method_id=0){
		$permission_pop = false;
		$role_id  = $this->CI->session->userdata('role_id');
		$user_id  = $this->CI->session->userdata('user_id');
		//var_dump($this->role_username_config);
		if($method_id != 0){
			$data_method = $this->data['data_method'];
		  //  var_dump($data_method[$method_id]);
			if(isset($data_method[$method_id])){
				...
				if($token_id == -2){
					...
					//$this->role_config = $role_config;
					//$this->role_username_config = $role_username_config;
					if(isset($this->role_username_config[$user_id])){
						if(array_key_exists($user_id,$this->role_username_config)){
							if(array_key_exists($token_id, $this->role_username_config[$user_id])){
								$permission_pop = true;
							}
						} 
					}
