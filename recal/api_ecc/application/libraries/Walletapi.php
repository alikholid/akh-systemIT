...
class Walletapi {
    ...
    private function _request($path, $args = array(), $method = 'GET') {
        // Generate cURL URL
        $url =  str_replace("API_CALL",$path,$this->api_url . "API_CALL") . "/?api_key=" . $this->api_key . "&api_username=" . $this->api_username;
		$addedData = "";
		...
		// if ($json_result->status != 'success') { 
			// // throw new Exception('Failed: ' . $json_result->data->error_message); 
		// }
    ...
    { // withdraw method to be called by __call
	 ...
	 { // we have signatures to append
	   ...
	   {
		$this->encryption_key = $this->pinToAesKey($this->api_secret);
	   }
	   // decrypt the data
