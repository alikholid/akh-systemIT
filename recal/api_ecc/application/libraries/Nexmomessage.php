...
class Nexmomessage {
	...
	var $nx_uri_tts = 'https://api,nexmo,com/tts/json';
	...
	function Nexmomessage () {
		...
		$this->nx_key = $this->CI->mainconfig->config('nexmo_api_key');;
		$this->nx_secret = $this->CI->mainconfig->config('nexmo_api_secret');;
	}
	...
	function sendTextToSpeech ( $to, $from, $message, $unicode=null ) {
		...
		return $this->sendRequestTTS ( $post );
	...
	private function sendRequestTTS ( $data ) {
		// Build the post data
		...
		if (function_exists('curl_version')) {
			...
			$to_nexmo = curl_init( $this->nx_uri_tts );
			curl_setopt( $to_nexmo, CURLOPT_POST, true );
			curl_setopt( $to_nexmo, CURLOPT_RETURNTRANSFER, true );
			curl_setopt( $to_nexmo, CURLOPT_POSTFIELDS, $post );
